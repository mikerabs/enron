...
def create_connection():
    try:
        connection = mysql.connector.connect(
            host='localhost',
            user='root',  
            password='password',#CHANGE THIS TO ACTUAL PASSWORD WHEN RUNNING 
            database='enron_emails'#DATABASE MUST BE CREATED FIRST
        )
...
def insert_data(connection, email_data):
    try:
        print(f"path data: {email_data['Folder-Path']}")
        cursor = connection.cursor()
        email_query = '''
            INSERT INTO Emails (message_id, date, subject, mime_version, content_type, content_transfer_encoding, x_folder, x_origin, x_filename, folder_path, content)
        ...
        cursor.execute(email_query, (
            ...
            email_data['X-Origin'], email_data['X-FileName'], email_data['Folder-Path'], email_data['Content']
        ))
...
def process_folder(folder_path, connection):
    ...
    for root, _, files in os.walk(folder_path):
        for email_file in files:
            ...
            folder_path_relative = str(os.path.relpath(root, base_dir))
            print(f"Relative path: {folder_path_relative}")
            message_content = []
            content_start = False
            for line in lines:
                line = line.strip()
